...
private const val DEF_SAMPLECOLOR = "vec3 sampleColor ( float u , float v ) { Ray ray = createRayFromTexCoord ( u , v ) ; HitRecord record = hitRayHitables ( ray , 0 , FLT_MAX ) ; float fraction = 1.0f ; for ( int i = 0 ; i < BOUNCES ; i ++ ) { if ( record . t <= 0 ) { break ; } vec3 target = addv3 ( addv3 ( record . point , record . normal ) , randomInUnitSphere ( ) ) ; Ray scattered = { record . point , subv3 ( target , record . point ) } ; record = hitRayHitables ( scattered , 0 , FLT_MAX ) ; fraction *= 0.5f ; } return mulv3f ( background ( ray ) , fraction ) ; }\n\n"
private const val DEF_SHADINGRT = "vec4 shadingRt ( vec2 texCoord ) { seedRandom ( texCoord ) ; vec3 result = v3zero ( ) ; float DU = 1.0f / WIDTH ; float DV = 1.0f / HEIGHT ; float SU = DU / SAMPLES ; float SV = DV / SAMPLES ; for ( int u = 0 ; u < SAMPLES ; u ++ ) { for ( int v = 0 ; v < SAMPLES ; v ++ ) { float sampleU = texCoord . x + SU * itof ( u ) ; float sampleV = texCoord . y + SV * itof ( v ) ; result = addv3 ( result , sampleColor ( sampleU , sampleV ) ) ; } } result = divv3f ( result , SAMPLES * SAMPLES ) ; return v3tov4 ( result , 1.0f ) ; }\n\n"
