...
private const val DEF_RAYFROMCAMERA = "Ray rayFromCamera ( RtCamera camera , float u , float v ) { if ( camera . lensRadius > 0.0f ) { } vec3 rd = mulv3f ( randomInUnitDisk ( ) , camera . lensRadius ) ; vec3 offset = addv3 ( mulv3f ( camera . u , rd . x ) , mulv3f ( camera . v , rd . y ) ) ; vec3 newOrigin = addv3 ( camera . origin , offset ) ; vec3 horisontal = mulv3f ( camera . horizontal , u ) ; vec3 vertical = mulv3f ( camera . vertical , v ) ; vec3 direction = normv3 ( subv3 ( subv3 ( addv3 ( camera . lowerLeft , addv3 ( horisontal , vertical ) ) , camera . origin ) , offset ) ) ; Ray result = { newOrigin , direction } ; return result ; }\n\n"
