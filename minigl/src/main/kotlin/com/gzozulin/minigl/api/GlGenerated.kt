...
private const val DEF_MATERIALSCATTERDIELECTRIC = "ScatterResult materialScatterDielectric ( Ray ray , HitRecord record , DielectricMaterial material ) { vec3 outwardNormal ; float niOverNt ; vec3 reflected = reflectv3 ( ray . direction , record . normal ) ; if ( dotv3 ( ray . direction , record . normal ) > 0 ) { outwardNormal = negv3 ( record . normal ) ; niOverNt = material . reflectiveIndex ; } else { outwardNormal = record . normal ; niOverNt = 1.0f / material . reflectiveIndex ; } RefractResult refractResult = refractv3 ( ray . direction , outwardNormal , niOverNt ) ; ScatterResult scatterResult = { { 1.0f , 1.0f , 1.0f } , { record . point , refractResult . isRefracted ? refractResult . refracted : reflected } } ; return scatterResult ; }\n\n"
