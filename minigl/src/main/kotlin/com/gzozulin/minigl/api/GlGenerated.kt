...
private const val DEF_CAMERALOOKAT = "RtCamera cameraLookAt ( vec3 eye , vec3 center , vec3 up , float vfoy , float aspect ) { float halfHeight = tan ( vfoy / 2.0f ) ; float halfWidth = aspect * halfHeight ; vec3 w = normv3 ( subv3 ( eye , center ) ) ; vec3 u = normv3 ( crossv3 ( up , w ) ) ; vec3 v = crossv3 ( w , u ) ; vec3 hwu = mulv3f ( u , halfWidth ) ; vec3 hhv = mulv3f ( v , halfHeight ) ; vec3 lowerLeft = subv3 ( subv3 ( subv3 ( eye , hwu ) , hhv ) , w ) ; vec3 horizontal = mulv3f ( u , halfWidth * 2.0f ) ; vec3 vertical = mulv3f ( v , halfHeight * 2.0f ) ; RtCamera result = { eye , lowerLeft , horizontal , vertical } ; return result ; }\n\n"
private const val DEF_RAYFROMCAMERA = "Ray rayFromCamera ( RtCamera camera , float u , float v ) { vec3 direction = normv3 ( subv3 ( addv3 ( camera . lowerLeft , addv3 ( mulv3f ( camera . horizontal , u ) , mulv3f ( camera . vertical , v ) ) ) , camera . origin ) ) ; Ray result = { camera . origin , direction } ; return result ; }\n\n"
...
fun sqlenv3(v: Expression<vec3>) = object : Expression<Float>() {
    override fun expr() = "sqlenv3(${v.expr()})"
    override fun roots() = listOf(v)
}
...
fun cameraLookAt(eye: Expression<vec3>, center: Expression<vec3>, up: Expression<vec3>, vfoy: Expression<Float>, aspect: Expression<Float>) = object : Expression<RtCamera>() {
    override fun expr() = "cameraLookAt(${eye.expr()}, ${center.expr()}, ${up.expr()}, ${vfoy.expr()}, ${aspect.expr()})"
    override fun roots() = listOf(eye, center, up, vfoy, aspect)
}
...
fun fragmentColorRt(sampleCnt: Expression<Int>, rayBounces: Expression<Int>, eye: Expression<vec3>, center: Expression<vec3>, up: Expression<vec3>, fovy: Expression<Float>, aspect: Expression<Float>, texCoord: Expression<vec2>) = object : Expression<vec4>() {
    override fun expr() = "fragmentColorRt(${sampleCnt.expr()}, ${rayBounces.expr()}, ${eye.expr()}, ${center.expr()}, ${up.expr()}, ${fovy.expr()}, ${aspect.expr()}, ${texCoord.expr()})"
    override fun roots() = listOf(sampleCnt, rayBounces, eye, center, up, fovy, aspect, texCoord)
}
