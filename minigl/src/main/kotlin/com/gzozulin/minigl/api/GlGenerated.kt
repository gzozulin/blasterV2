...
fun raymarcher(eye: Expression<vec3>, center: Expression<vec3>, uv: Expression<vec2>, fovy: Expression<Float>, aspect: Expression<Float>, wh: Expression<vec2i>, samplesAA: Expression<Int>, sphereO: Expression<vec3>, sphereR: Expression<Float>, cylStart: Expression<vec3>, cylStop: Expression<vec3>, cylR: Expression<Float>, boxO: Expression<vec3>, boxR: Expression<vec3>, coneO: Expression<vec3>, coneS: Expression<vec2>, coneH: Expression<Float>, prismO: Expression<vec3>, prismS: Expression<vec2>) = object : Expression<vec4>() {
    override fun expr() = "raymarcher(${eye.expr()}, ${center.expr()}, ${uv.expr()}, ${fovy.expr()}, ${aspect.expr()}, ${wh.expr()}, ${samplesAA.expr()}, ${sphereO.expr()}, ${sphereR.expr()}, ${cylStart.expr()}, ${cylStop.expr()}, ${cylR.expr()}, ${boxO.expr()}, ${boxR.expr()}, ${coneO.expr()}, ${coneS.expr()}, ${coneH.expr()}, ${prismO.expr()}, ${prismS.expr()})"
    override fun roots() = listOf(eye, center, uv, fovy, aspect, wh, samplesAA, sphereO, sphereR, cylStart, cylStop, cylR, boxO, boxR, coneO, coneS, coneH, prismO, prismS)
}
