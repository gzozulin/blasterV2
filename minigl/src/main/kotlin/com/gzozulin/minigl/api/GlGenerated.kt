...
private const val DEF_MAX_STEPS = "int MAX_STEPS = 1000 ;\n"
private const val DEF_MAX_DIST = "float MAX_DIST = 100.0f ;\n"
private const val DEF_SURF_DIST = "float SURF_DIST = 0.0001f ;\n"
...
private const val DEF_SIMPLIFIEDCYL = "float simplifiedCyl ( vec3 p , float cylLen , float cylRad ) { return sdCappedCylinder ( p , v3 ( 0 , 0 , cylLen / 2.0f ) , v3 ( 0 , 0 , - cylLen / 2.0f ) , cylRad ) ; }\n"
private const val DEF_SCENEDIST = "float sceneDist ( vec3 p , RaymarcherScene scene ) { vec4 p4 = v3tov4 ( p , 1.0f ) ; vec3 cylAP = v4tov3 ( transformv4 ( p4 , scene . cylAMat ) ) ; float cylA = simplifiedCyl ( cylAP , scene . cylALen , scene . cylARad ) ; vec3 coneBP = v4tov3 ( transformv4 ( p4 , scene . coneBMat ) ) ; float coneB = sdCone ( coneBP , scene . coneBShape , scene . coneBHeight ) ; vec3 cylCP = v4tov3 ( transformv4 ( p4 , scene . cylCMat ) ) ; float cylC = simplifiedCyl ( cylCP , scene . cylCLen , scene . cylCRad ) ; vec3 boxDP = v4tov3 ( transformv4 ( p4 , scene . boxDMat ) ) ; float boxD = sdBox ( boxDP , scene . boxDShape ) ; vec3 boxEP = v4tov3 ( transformv4 ( p4 , scene . boxEMat ) ) ; float boxE = sdBox ( boxEP , scene . boxEShape ) ; vec3 prismFP = v4tov3 ( transformv4 ( p4 , scene . prismFMat ) ) ; float prismF = sdTriPrism ( prismFP , scene . prismFShape ) ; vec3 cylGP = v4tov3 ( transformv4 ( p4 , scene . cylGMat ) ) ; float cylG = simplifiedCyl ( cylGP , scene . cylGLen , scene . cylGRad ) ; vec3 boxHP = v4tov3 ( transformv4 ( p4 , scene . boxHMat ) ) ; float boxH = sdBox ( boxHP , scene . boxHShape ) ; float AB = opSubtraction ( coneB , cylA ) ; float AC = opUnion ( AB , cylC ) ; float AD = opUnion ( AC , boxD ) ; float AE = opUnion ( AD , boxE ) ; float AF = opUnion ( AE , prismF ) ; float AG = opUnion ( AF , cylG ) ; float AH = opSubtraction ( boxH , AG ) ; return AH ; }\n"
