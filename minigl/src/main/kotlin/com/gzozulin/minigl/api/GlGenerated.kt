...
private const val DEF_ERRORHANDLER = "vec4 errorHandler ( vec4 color ) { if ( errorFlag ) { vec3 signal ; float check = randf ( ) ; if ( check > 0.6f ) { signal = v3red ( ) ; } else if ( check > 0.3f ) { signal = v3blue ( ) ; } else { signal = v3green ( ) ; } return v3tov4 ( signal , 1.0f ) ; } else { return color ; } }\n\n"
...
private const val DEF_RAYHITOBJECT = "HitRecord rayHitObject ( Ray ray , float tMin , float tMax , int type , int index ) { switch ( type ) { case HITABLE_SPHERE : return rayHitSphere ( ray , tMin , tMax , uSpheres [ index ] ) ; default : error ( ) ; return NO_HIT ; } }\n\n"
private const val DEF_RAYHITBVH = "HitRecord rayHitBvh ( Ray ray , float tMin , float tMax , int index ) { float closest = tMax ; HitRecord result = NO_HIT ; int stack [ MAX_BVH ] ; int top = 0 ; int curr = index ; while ( curr != HITABLE_NONE || top > 0 ) { while ( curr != HITABLE_NONE && rayHitAabb ( ray , uBvhNodes [ curr ] . aabb , tMin , closest ) ) { if ( uBvhNodes [ curr ] . leftType == HITABLE_BVH ) { stack [ top ] = curr ; top ++ ; curr = uBvhNodes [ curr ] . leftIndex ; } else { HitRecord hit = rayHitObject ( ray , tMin , closest , uBvhNodes [ curr ] . leftType , uBvhNodes [ curr ] . leftIndex ) ; if ( hit . t > 0 && hit . t < closest ) { result = hit ; closest = hit . t ; } break ; } } top -- ; curr = stack [ top ] ; curr = uBvhNodes [ curr ] . rightIndex ; } return result ; }\n\n"
...
fun fragmentColorRt(random: Expression<Float>, sampleCnt: Expression<Int>, rayBounces: Expression<Int>, eye: Expression<vec3>, center: Expression<vec3>, up: Expression<vec3>, fovy: Expression<Float>, aspect: Expression<Float>, aperture: Expression<Float>, focusDist: Expression<Float>, texCoord: Expression<vec2>) = object : Expression<vec4>() {
    override fun expr() = "fragmentColorRt(${random.expr()}, ${sampleCnt.expr()}, ${rayBounces.expr()}, ${eye.expr()}, ${center.expr()}, ${up.expr()}, ${fovy.expr()}, ${aspect.expr()}, ${aperture.expr()}, ${focusDist.expr()}, ${texCoord.expr()})"
    override fun roots() = listOf(random, sampleCnt, rayBounces, eye, center, up, fovy, aspect, aperture, focusDist, texCoord)
}
