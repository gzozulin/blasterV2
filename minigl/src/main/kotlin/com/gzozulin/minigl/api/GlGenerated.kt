...
private const val DEF_SANDPHYSICS = "vec4 sandPhysics ( sampler2D orig , vec2 uv , ivec2 wh ) { float cellW = 1.0f / itof ( wh . x ) ; float cellH = 1.0f / itof ( wh . y ) ; vec4 dummy = sampler ( orig , v2 ( cellW * uv . x , cellH * uv . y ) ) ; return v4 ( 0.0f , 0.0f , dummy . x , dummy . y ) ; }\n"
private const val DEF_SANDSOLVER = "vec4 sandSolver ( sampler2D orig , sampler2D deltas , vec2 uv , ivec2 wh ) { float cellW = 1.0f / itof ( wh . x ) ; float cellH = 1.0f / itof ( wh . y ) ; vec4 result = v4zero ( ) ; for ( int x = - 1 ; x < 2 ; x ++ ) { for ( int y = - 1 ; y < 2 ; y ++ ) { vec2 coords = v2 ( uv . x + itof ( x ) * cellW , uv . y + itof ( y ) * cellH ) ; vec4 curr = sampler ( orig , coords ) ; if ( curr . x == 0 ) { continue ; } vec4 delta = sampler ( deltas , coords ) ; if ( delta . x == itof ( - x ) && delta . y == itof ( - y ) ) { addv4 ( result , v4one ( ) ) ; } } } return result ; }\n"
