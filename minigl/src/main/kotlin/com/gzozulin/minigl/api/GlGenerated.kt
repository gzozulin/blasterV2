...
private const val DEF_TRYDEPOSITPARTICLE = "ivec2 tryDepositParticle ( sampler2D orig , vec2 uv , float cellW , float cellH , int x ) { vec2 coords = nearbyCellCoords ( uv , cellW , cellH , x , - 1 ) ; if ( coords . x < 0.0f || coords . y < 0.0f || coords . x > 1.0f || coords . y > 1.0f ) { return iv2zero ( ) ; } vec4 below = sampler ( orig , coords ) ; if ( eqv4 ( below , v4zero ( ) ) ) { return iv2 ( x , - 1 ) ; } return iv2zero ( ) ; }\n"
