...
private const val DEF_RAYSCATTERLAMBERTIAN = "ScatterResult rayScatterLambertian ( HitRecord record , LambertianMaterial material ) { vec3 tangent = addv3 ( record . point , record . normal ) ; vec3 direction = addv3 ( tangent , randomInUnitSphere ( ) ) ; ScatterResult result = { material . albedo , { record . point , subv3 ( direction , record . point ) } } ; return result ; }\n\n"
...
private const val DEF_RAYHITWORLD = "HitRecord rayHitWorld ( Ray ray , float tMin , float tMax ) { HitRecord result = NO_HIT ; float closest = tMax ; for ( int i = 0 ; i < uHitablesCnt ; i ++ ) { Hitable hitable = uHitables [ i ] ; HitRecord hitRecord ; switch ( hitable . type ) { case HITABLE_SPHERE : hitRecord = rayHitSphere ( ray , tMin , closest , uSpheres [ hitable . index ] ) ; break ; default : hitRecord = NO_HIT ; } if ( hitRecord . t > 0 ) { closest = hitRecord . t ; result = hitRecord ; } } return result ; }\n\n"
private const val DEF_SAMPLECOLOR = "vec3 sampleColor ( float u , float v ) { Ray ray = rayFromTexCoord ( u , v ) ; vec3 fraction = ftov3 ( 1.0f ) ; for ( int i = 0 ; i < BOUNCES ; i ++ ) { HitRecord record = rayHitWorld ( ray , BOUNCE_ERR , FLT_MAX ) ; if ( record . t < 0 ) { break ; } else { switch ( record . materialType ) { case MATERIAL_LAMBERTIAN : { ScatterResult scatterResult = rayScatterLambertian ( record , uLambertianMaterials [ record . materialIndex ] ) ; fraction = mulv3 ( fraction , scatterResult . attenuation ) ; ray = scatterResult . scattered ; break ; } } } } return mulv3 ( background ( ray ) , fraction ) ; }\n\n"
...
fun rayScatterLambertian(record: Expression<HitRecord>, material: Expression<LambertianMaterial>) = object : Expression<ScatterResult>() {
    override fun expr() = "rayScatterLambertian(${record.expr()}, ${material.expr()})"
...
fun sampleColor(u: Expression<Float>, v: Expression<Float>) = object : Expression<vec3>() {
    override fun expr() = "sampleColor(${u.expr()}, ${v.expr()})"
    override fun roots() = listOf(u, v)
}

fun fragmentColorRt(texCoord: Expression<vec2>) = object : Expression<vec4>() {
    override fun expr() = "fragmentColorRt(${texCoord.expr()})"
