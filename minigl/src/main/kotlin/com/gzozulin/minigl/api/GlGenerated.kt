...
private const val DEF_RAYFROMTEXCOORD = "Ray rayFromTexCoord ( float u , float v ) { vec3 lowerLeft = { - 1 , - 1 , - 1 } ; vec3 origin = { 0 , 0 , 0 } ; vec3 horizontal = { 2 , 0 , 0 } ; vec3 vertical = { 0 , 2 , 0 } ; vec3 direction = normv3 ( addv3 ( lowerLeft , addv3 ( mulv3f ( horizontal , u ) , mulv3f ( vertical , v ) ) ) ) ; Ray result = { origin , direction } ; return result ; }\n\n"
...
fun rayFromTexCoord(u: Expression<Float>, v: Expression<Float>) = object : Expression<ray>() {
    override fun expr() = "rayFromTexCoord(${u.expr()}, ${v.expr()})"
...
fun sampleColor(rayBounces: Expression<Int>, u: Expression<Float>, v: Expression<Float>) = object : Expression<vec3>() {
    override fun expr() = "sampleColor(${rayBounces.expr()}, ${u.expr()}, ${v.expr()})"
    override fun roots() = listOf(rayBounces, u, v)
}

fun fragmentColorRt(sampleCnt: Expression<Int>, rayBounces: Expression<Int>, texCoord: Expression<vec2>) = object : Expression<vec4>() {
    override fun expr() = "fragmentColorRt(${sampleCnt.expr()}, ${rayBounces.expr()}, ${texCoord.expr()})"
    override fun roots() = listOf(sampleCnt, rayBounces, texCoord)
}
