...
private const val DEF_RAYHITBVH = "HitRecord rayHitBvh ( Ray ray , float tMin , float tMax , int index ) { float closest = tMax ; HitRecord result = NO_HIT ; int stack [ MAX_BVH ] ; int top = 0 ; int curr = index ; while ( curr >= 0 ) { while ( curr >= 0 && rayHitAabb ( ray , uBvhNodes [ curr ] . aabb , tMin , closest ) ) { if ( uBvhNodes [ curr ] . leftType == HITABLE_BVH ) { stack [ top ] = curr ; top ++ ; curr = uBvhNodes [ curr ] . leftIndex ; } else { HitRecord hit = rayHitObject ( ray , tMin , closest , uBvhNodes [ curr ] . leftType , uBvhNodes [ curr ] . leftIndex ) ; if ( hit . t > 0 && hit . t < closest ) { result = hit ; closest = hit . t ; } break ; } } top -- ; if ( top < 0 ) { break ; } curr = stack [ top ] ; curr = uBvhNodes [ curr ] . rightIndex ; } return result ; }\n\n"
